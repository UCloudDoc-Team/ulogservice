...
检索分析语句由检索条件和 SQL 语句组成，两者通过竖线|分割，针对符合检索条件的日志进行统计分析，返回统计分析结果。
```
[检索条件] | [SQL 语句]
```
- **SQL 语句**：针对符合检索条件的日志进行统计分析，返回统计分析结果。例如使用 `level:error | SELECT COUNT(*) as cnt` 统计级别为 error 的日志数量。


## 分析语法

日志分析支持类 SQL 语法，可用于对日志数据进行统计分析。

### 操作步骤
1. 登录 日志服务控制台。
2. 在顶部的栏目中选择检索分析。
3. 在左侧选择要执行分析。
4. 选择检索分析语句输入模式，日志服务提供两种模式来输入检索分析语句。
